...
const HealthyWeightCalculator = () => {
  ...
  const calculate = (e) => {
    ...
    if (!h) return;
    let hMeters = h / 100;
    let minWeight = 18.5 * (hMeters * hMeters);
    let maxWeight = 24.9 * (hMeters * hMeters);
    setResult({ min: minWeight.toFixed(1), max: maxWeight.toFixed(1) });
  };
    ...
    <>
      ...
      <div className="page-hero">
        <div className="container">
          ...
          <p>Determine your optimal healthy weight range.</p>
        </div>
      </div>
      ...
      <main className="container tool-layout-container" style={{ paddingTop:'2rem', paddingBottom:'4rem' }}>
        ...
        <div className="main-content">
          ...
          <div className="calculator-card">
            <div className="mac-header">
              ...
              <div className="mac-title">HEALTHY WEIGHT CALCULATOR</div>
            </div>
              ...
              <div className="calculator-grid">
                ...
                <div className="results-container" style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: '300px' }}>
                  ...
                  <div style={{ background: '#fff', flex: 1, padding: '2rem 1.5rem', borderBottomLeftRadius: 'var(--r-lg)', borderBottomRightRadius: 'var(--r-lg)', display: 'flex', flexDirection: 'column', justifyContent: result ? 'flex-start' : 'center', alignItems: 'center', border: '1px solid var(--border)' }}>
                    ...
                    <div style={{ fontSize: '.85rem', textTransform: 'uppercase', letterSpacing: '.05em', color: 'var(--text-2)', fontWeight: 600, marginBottom: '.5rem' }}>Healthy Weight Range</div>
                    <div style={{ fontSize: '2.5rem', fontWeight: 800, color: 'var(--green)', lineHeight: 1 }}>{result.min} - {result.max}</div>
                    <div style={{ fontSize: '1rem', color: 'var(--muted)', marginTop: '.5rem' }}>kg</div>
                      </div>
                    )}
                  </div>
                </div>
        ...
        <article className="seo-article-content" style={{ marginTop: '4rem', padding: '2rem', background: '#fff', borderRadius: 'var(--r-xl)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
          <header style={{ marginBottom: '2rem', textAlign: 'center' }}>
            <h2 style={{ fontSize: '2.2rem', color: 'var(--text)', marginBottom: '1rem' }}>The Ultimate Guide to Your Healthy Weight Calculator</h2>
            <p style={{ fontSize: '1.1rem', color: 'var(--text-2)', lineHeight: 1.8, maxWidth: '800px', margin: '0 auto' }}>
              The Healthy Weight Calculator calculator is a specialized fitness and health tool designed to provide highly accurate metrics to optimize your daily routine, nutrition, and training program. Discover how to leverage this metric to accelerate your health, fitness, and nutritional goals using verified scientific data.
            </p>
          </header>

          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', marginBottom: '3rem', justifyContent: 'center' }}>
            {["healthy weight calculator calculator","calculate healthy weight calculator","free healthy weight calculator tool","accurate healthy weight calculator","health and fitness metrics"].map((kw, i) => (
          ...
          <div className="seo-content-grid" style={{ display: 'flex', flexDirection: 'column', gap: '2.5rem' }}>
            ...
            <section>
              <h3 style={{ fontSize: '1.5rem', color: 'var(--blue-dark)', marginBottom: '1rem', borderBottom: '2px solid var(--green-light)', paddingBottom: '0.5rem', display: 'inline-block' }}>Comprehensive Guide to the Healthy Weight Calculator</h3>
              <p style={{ fontSize: '1.05rem', color: 'var(--text-2)', lineHeight: 1.8 }}>Understanding your Healthy Weight Calculator is critical for optimizing your physical health and reaching your specific fitness goals. Whether your objective is to build lean muscle mass, maximize fat loss, improve athletic endurance, or simply maintain a healthy lifestyle, accurate data is your most powerful asset. Our professional-grade Healthy Weight Calculator calculator utilizes peer-reviewed scientific formulas to deliver precise, actionable insights tailored specifically to your unique physiological profile.</p>
            </section>
